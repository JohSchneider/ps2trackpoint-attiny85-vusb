...
#define CLAMP(x) ( x < - 128 ? - 128 : (x > 127 ? 127 : x) )

static void updateMouseReport(void)
{
    reportBuffer.buttonMask = ps2mouse_b;

    reportBuffer.dx = CLAMP(ps2mouse_x);
    reportBuffer.dy = CLAMP(ps2mouse_y);
    ps2mouse_x -= reportBuffer.dx;
    ps2mouse_y -= reportBuffer.dy;
...
// the Adafruit Trinket comes with a red status led connected to B1
#define ledRedOn()    \
    {                 \
    PORTB |= _BV(1); \
    DDRB |= _BV(1);   \
